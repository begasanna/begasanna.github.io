...
localStorage.setItem('test',JSON.stringify(myLodash)); //обратила в строку и отправила в хранилище
var myTest = localStorage.getItem('test'); // достала из хранилища
var objLodash = JSON.parse(myTest); // распарсила
// Выборка скиллов
var skills3 = _.map(objLodash, 'skills'); //выборка массива по ключу
var skills2 = _.flatten(skills3, 'skills'); // извлекаем, в виде массива, элементы из объектов
var skills1 = _.uniq(skills2, 'skills'); // убираем повторы
var skills = _.sortBy(skills1);
// var skills = skills1.sort();
console.log('Отсортированный массив скиллов:',skills);

//Выборка людей по количеству друзей

var names3 = _.orderBy(objLodash, 'friends.length');
var names2 = _.map(names3,'name');
var names1 = _.map(names3,'friends.length');
var names = _.zipObject(names2,names1);
console.log('All users with our friends',names);



//Выборка имен друзей
var friend3 = _.map(objLodash, 'friends'); // получаем обїект с массивами по ключу
var friend2 = _.flatten(friend3, 'name');  // получаем обЪекты из массива по ключу
var friend1 = _.map(friend2, 'name'); // получаем массив значений по ключу
var friend = _.uniq(friend1); // убираем повторы
console.log('all friends:',friend);
